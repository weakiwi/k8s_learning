...
func Run(c *cli.Context) {
	...
	for _, ad := range addresses {
		fmt.Println(fmt.Sprintf("nslookup for %s ...", ad))
		for {
			err := portCheck(ad)
			if err != nil {
				log.Println(err)
			} else {
				fmt.Println(fmt.Sprintf("find domain %s successfully", ad))
				break
			}
...
func portCheck(address string) error {
	...
	if err, ok := err.(net.Error); ok && err.Timeout() {
		return errors.New(fmt.Sprintf("timeout resolve domain: %+v", err))
	}
	return errors.New(fmt.Sprintf("failure to resolve domain: %+v", err))
}
